...
public class GestureInterpreter : MonoBehaviour
{
	...
	private const double DRAW_MODIFIER = 11;
	...
	public Vector3 LastTapPosition{
		get{
			return Camera.main.ScreenToWorldPoint(lastTapPosition);
		}
		set{}
	}
	...
	public LinkedList<Vector3> LinePoints{
		get{
			return linePoints;
		}
		set{}
    ...
    {
		if (checkDrawUpdateDistanceReached(Input.mousePosition)){
        	updateLine();
        }
	...
	private bool checkDrawUpdateDistanceReached(Vector3 mousePosition){
        return Mathf.Sqrt(Mathf.Pow(pathPosition.Last.Value.x - mousePosition.x, 2) + Mathf.Pow(pathPosition.Last.Value.y - Input.mousePosition.y, 2)) > getDrawUpdateDistance();
    }
	...
	private double getDrawUpdateDistance(){
		return (Screen.width > Screen.height) ? Screen.height / DRAW_MODIFIER : Screen.width / DRAW_MODIFIER;
	}
    ...
    private LinkedList<direction> removeDuplicates(LinkedList<direction> path1)
    {
        ...
        foreach (GestureInterpreter.direction direction1 in path1)
        {
            if (direction1 != last)
            {
